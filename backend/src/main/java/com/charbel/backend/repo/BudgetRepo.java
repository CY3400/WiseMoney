...
public interface BudgetRepo extends JpaRepository<Budget, Long> {
    ...
    INNER JOIN WISE_MONEY.CATEGORIES C ON C.ID = T.CATEGORY_ID) SUB
    WHERE USER_ID = :userId
    GROUP BY MONTH, YEAR, USER_ID) SUB2
    INNER JOIN WISE_MONEY.BUDGETS B ON B.USER_ID = SUB2.USER_ID AND B.MONTH = SUB2.MONTH AND B.YEAR = SUB2.YEAR
