...
public interface TransactionRepo extends JpaRepository<Transaction, Long> {
    ...
    @Query(value="""
                ...
                FROM WISE_MONEY.TRANSACTIONS T
                INNER JOIN WISE_MONEY.BUDGET_MANAGEMENT BM ON BM.CATEGORY_ID = BM.CATEGORY_ID
    ...
    List<ChildPercentView> findChildrenSpendViewByUserId(@Param("userId") Long userId, @Param("parentId") Long parentId);
        ...
        FROM (SELECT T.USER_ID, T.CATEGORY_ID, GREATEST(CASE WHEN T.CATEGORY_ID IN (SELECT CATEGORY_ID FROM wise_money.transactions WHERE USER_ID = :userId AND EXTRACT(MONTH FROM TRANSACTION_DATE) = EXTRACT(MONTH FROM SYSDATE()) AND EXTRACT(YEAR FROM TRANSACTION_DATE) = EXTRACT(YEAR FROM SYSDATE())) THEN (SELECT SUM(AMOUNT) FROM TRANSACTIONS WHERE USER_ID = :userId AND EXTRACT(MONTH FROM TRANSACTION_DATE) = EXTRACT(MONTH FROM SYSDATE()) AND EXTRACT(YEAR FROM TRANSACTION_DATE) = EXTRACT(YEAR FROM SYSDATE()) AND CATEGORY_ID = T.CATEGORY_ID) ELSE 0 END, ROUND(TB.TOTAL * BM.AMOUNT / 100, 0)) AS TOTAL
        FROM WISE_MONEY.TRANSACTIONS T
        INNER JOIN WISE_MONEY.BUDGET_MANAGEMENT BM ON BM.CATEGORY_ID = T.CATEGORY_ID
        INNER JOIN TOTAL_BUDGET TB ON TB.USER_ID = BM.USER_ID
